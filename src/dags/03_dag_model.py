...
def metadata_load(**context):
    ...
    split_metadata = ti.xcom_pull(dag_id='02_dag_preprocess', task_ids='metadata_load', key='split_metadata')
    ...
    split_type = split_metadata.get('split_type')
    ...
    if split_type == 'simple':
        ...
        num_folds = split_metadata.get('num_folds', 5)
    ...
    return f"Loaded {split_type} split metadata with {len(folds_info)} fold(s)"
...
def model_register(**context):
    ...
    print(f"Best {best_metric_key}: {best_fold['metrics'][best_metric_key]:.4f}")
    ...
    registration_tags = {
        ...
        "best_metric_value": str(best_fold['metrics'][best_metric_key]),
    ...
    registration_info = {
        ...
        'best_metric': best_metric_key,
        'best_metric_value': best_fold['metrics'][best_metric_key],
        'task_type': task_type,
    ...
    print(f"  Best fold: {best_fold['fold_id']}")
    print(f"  {best_metric_key}: {best_fold['metrics'][best_metric_key]:.4f}")
    print(f"{'='*60}")
...
with DAG(
    ...
    train_fold_model_tasks = train_fold_model.expand(
        fold_info="{{ ti.xcom_pull(task_ids='metadata_load', key='folds_info') }}"
    ).partial(
