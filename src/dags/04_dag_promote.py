"""
04_dag_promote - Model Promotion DAG
...
- Emits PROMOTED_MODEL_ASSET to trigger DAG 05 (FastAPI reload)

Model serving is handled by the FastAPI service (model-serve container)
which loads champion models from MLflow and serves predictions at /predict/{model_name}.
"""
...
# Import assets
from assets import TRAINED_MODEL_ASSET, PROMOTED_MODEL_ASSET
...
def load_model_metadata(**context):
    ...
    1. XCom from DAG 03 (if available)
    2. Direct query to MLflow Model Registry (fallback)
    """
    ti = context['ti']
    promotion_config = getattr(config, 'PROMOTION', {})
    model_name = promotion_config.get('MODEL_NAME', 'ml_pipeline_model')

    # Initialize MLflow logger for fallback queries
    ...
    if not registration_info:
        print("XCom pull failed, querying MLflow Model Registry directly...")
        latest_version = logger.get_latest_model_version(model_name)

        if latest_version:
            # Get metrics from the model's run
            run_metrics = logger.get_model_version_metrics(latest_version['run_id'])

            # Determine task type and best metric from run metrics
            ...
            registration_info = {
                ...
                'task_type': task_type,
                'source': 'mlflow_registry'  # Mark that this came from registry query
            }
            print(f"Found latest model version from registry: v{latest_version['version']}")
        else:
...
def get_champion_model(**context):
    ...
    # Get champion model version by alias
    champion_info = logger.get_model_version_by_alias(model_name, champion_alias)

    if champion_info:
        # Get champion model's metrics from its training run
        ...
        print(f"\nNo champion model found (alias '{champion_alias}' not set)")
        print("This appears to be the first model deployment.")
...
def compare_models(**context):
    ...
    Determines if the new model should be promoted based on:
    - Metric comparison (higher/lower is better depending on config)
    - Minimum improvement threshold
    - Auto-promote first model setting
    """
    ti = context['ti']
    ...
    # Get new model's metric value from registration_info
    ...
    if champion_info is None:
        # No champion exists - this is the first deployment
...
def promote_model(**context):
    ...
    If not promoting, sets challenger alias instead.
    """
    ti = context['ti']
    ...
    if not comparison_result['should_promote']:
        print(f"Skipping promotion: {comparison_result['reason']}")
        # Set challenger alias instead
    ...
    if success:
        ...
        print(f"ERROR: Failed to set champion alias")
...
def log_promotion_results(**context):
    ...
    Creates a comprehensive promotion run with all metadata for traceability.
    """
    ti = context['ti']
    ...
    try:
        ...
        report_lines.extend([
            ...
            "",
            "SERVING:",
            f"  FastAPI service will reload via DAG 05 (05_dag_reload_serve)",
        ...
        print(f"Error logging promotion results: {e}")
...
with DAG(
    ...
    description="Model promotion DAG - compares new models against champion and promotes better-performing model",
    schedule=[TRAINED_MODEL_ASSET],  # Trigger when 03_dag_model completes
    start_date=datetime(2026, 1, 1),
    catchup=False,
    tags=["model-promotion", "ml-pipeline"],
) as dag:

    # Task 1: Load model metadata from DAG 03
    load_metadata_task = PythonOperator(
        task_id="load_model_metadata",
        python_callable=load_model_metadata,
    )

    # Task 2: Get current champion model
    get_champion_task = PythonOperator(
        task_id="get_champion_model",
        python_callable=get_champion_model,
    )
    ...
    # Task 4: Promote model if better (emits PROMOTED_MODEL_ASSET to trigger DAG 05)
    promote_task = PythonOperator(
        task_id="promote_model",
        python_callable=promote_model,
        outlets=[PROMOTED_MODEL_ASSET],  # Triggers 05_dag_reload_serve
    )

    # Task 5: Log promotion results
    log_results_task = PythonOperator(
        task_id="log_promotion_results",
        python_callable=log_promotion_results,
    )
